...
void solve() {
  ...
  for1(i, n) { idx[abs(p[i] - p[i - 1])].pb(i); }
  ...
  for (int diff = n - 1; diff > 0; diff--) {
    for (auto i : idx[diff]) {
      int rootx = dsu_find(i);
      int rooty = dsu_find(i - 1);
      dbg(i, rootx, sz[rootx]);
      dbg(i - 1, rooty, sz[rooty]);
      dbg(cur, "prev prev sum");
      cur -= f(sz[rootx]);
      cur -= f(sz[rooty]);
      dsu_merge(i, i - 1);

      dbg(cur, "prev sum");
      cur += f(sz[dsu_find(rootx)]);
      dbg(cur, "aft sum", dsu_find(rootx), sz[dsu_find(rootx)]);
    }
    ans[diff] = cur;
  }
  for1(i, n) cout << ans[i] << " ";
