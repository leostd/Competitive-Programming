...
string digit;
ll dp(int i, bool bounded) {
  if (i == n)
    return 0;

  ll &ret = memo[i][bounded];
  if (ret != -1)
    return ret;

  ret = 0;
  ll d = digit[i] - '0';
  forn(i, (bounded ? d + 1 : 10)) {
    if (bounded && i == d) {
      ll suf = (i < n-1 ? stoll(digit.substr(i+1, n) : 0));
      ret += d * (suf + 1);
    }
  }
}
...
void solve() {
  ...
  ll num = cur / 9 + (k - 1) / len;
  digit = to_string(num - 1);
  n = sz(digit);
  ll ans = dp(0, true);
  string aux = to_string(num);
  forn(i, k % len) { ans += aux[i] - '0'; }
