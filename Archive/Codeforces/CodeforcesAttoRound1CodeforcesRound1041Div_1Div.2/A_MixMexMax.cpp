// time-limit: 1000
// problem-url: https://codeforces.com/contest/2127/problem/A
//
...
bool validate(vector<int> x) {
  ...
  return mex == (x.back() - x.front());
}

bool f(vector<int> x, int idx) {
  int cnt = 0;
  vector<int> nx;
  vector<int> jdx;
  forn(i, sz(x)) {
...
void solve() {
  ...
  forn(i, n - 2) {
    vector<int> aux = {a[i], a[i + 1], a[i + 2]};
    yes = yes && f(aux, i);
