// time-limit: 2000
// problem-url: https://codeforces.com/contest/2185/problem/F
...
void solve() {
  ...
  forn(i, n) cin >> a[i];
  vector<int> pre(n + 1);
  for1(i, n + 1) pre[i] = pre[i - 1] ^ a[i - 1];
  int q = nxt();
  while (q--) {
    int x, y;
    cin >> x >> y;
    ...
    forn(pw, cows) {
      ...
      int nblocks = n / sze;
      int bidx = x / sze;
      dbg(sze, nblocks, bidx);
      int l = bidx * sze;
      int r = l + sze;
      dbg(l, r);
      int xr = pre[r] ^ pre[max(0, l - 1)];
      xr ^= a[x];
      xr ^= y;
      dbg(xr);
      int enemyl = (bidx % 2 ? l - sze : r);
      int enemyr = enemyl + sze;
      dbg(enemyl, enemyr);
      int enemyxr = pre[enemyr] ^ pre[max(0, enemyl - 1)];
      dbg(enemyxr);
      if (enemyxr > xr)
        curpos += sze;
      else if (enemyxr == xr && bidx % 2)
        curpos += sze;
      dbg(cur);
    }
    cout << curpos << endl;
