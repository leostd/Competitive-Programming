// time-limit: 2000
// problem-url: https://codeforces.com/contest/2185/problem/G
...
void solve() {
  ...
  vector<int> mex(n), secondMex(n);
  map<int, int> fq;
  vector<map<int, int>> fqq;
  map<int, vector<int>> auxx;
  ll sum_mex = 0;
  ...
  forn(i, n) {
    forn(j, sz(a[i])) {
      ans += (ll)sum_mex * (ll)(n - 1);
      // removing a[i][j]
      // if a[i][j] <= mex and fqq[i][a[i][j]] == 1 mex = a[i][j]
      bool is_rep = fqq[i][a[i][j]] > 1;
      if (!is_rep && a[i][j] <= mex[i]) {
        ans += (ll)a[i][j] * (ll)(n - 1);
        ans -= (ll)mex[i] * (ll)(n - 1);
      }
      ...
      // adding a[i][j]
      if (fq[a[i][j]]) {
        ans -= (ll)fq[a[i][j]] * (ll)(a[i][j]);
        for (auto idx : auxx[a[i][j]])
          ans += secondMex[idx];
      }
    }
  }
