...
ll find_ans(const vector<ll> &psum, ll sw, ll x) {
  int aux = upper_bound(all(psum), sw) - psum.begin() - 1;
  aux = max(0, aux);
  dbg(psum);
  dbg(sw, x);
  dbg(aux);
...
void solve() {
  ...
  vector<ll> psum(n + 1);
  for1(i, n + 1) psum[i] = psum[i - 1] + b[i - 1];

  ll ans = find_ans(psum, n, cnt.begin()->fst);

  ll less = 0;
  for (auto x : cnt) {
    ans = max(ans, find_ans(psum, n - less, x.fst));
    less += x.snd;
  }
