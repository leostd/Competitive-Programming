...
ll memo[MAXN], memo2[MAXN];
vector<int> par;

ll sum(ll a, ll b) { return ((a % MOD) + (b % MOD)) % MOD; }

// f[u] = pasos para recorrer la subárbol de u y volver al padre
ll dp(int root) {
  if (g[root].empty())
    return 1; // hoja: 1 paso (volver al padre)

  ll &ret = memo[root];
  if (ret != -1)
    return ret;

  ll left = (int)g[root].size() >= 1 ? dp(g[root][0]) : 0LL;
  ll right = (int)g[root].size() == 2 ? dp(g[root][1]) : 0LL;
  ret = sum(sum(left, right), 3LL);
  return ret;
}

// suma de f[v] en el camino de u hasta la raíz (nodo 1) → respuesta para u
ll dp2(int root) {
  if (par[root] == 0)
    return dp(root); // nodo 1: solo f[1]

  ll &ret = memo2[root];
  if (ret != -1)
    return ret;

  ret = sum(dp(root), dp2(par[root]));
  return ret;
}
