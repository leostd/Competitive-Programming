...
public:
    ...
    Matrix(int _n, int _m, bool identity = false) {
        nrow = _n;
        ncol = _m;
        m.assign(nrow, vector<ll>(ncol, 0));
    ...
    inline Matrix modMult(Matrix &b, ll &mod) {
        ...
        for(int i = 0; i < adim.first; i++) {
            for(int j = 0; j < bdim.second; j++) {
                for(int k = 0; k < adim.first; k++) {
                    // c[i][j] = ((c[i][j]%mod) + (((m[i][k]%mod) * (b[k][j]%mod))%mod))%mod;
                    c.set(i,j, ((c.get(i, j)%mod) + ((m[i][k]%mod) * (b.get(k, j)%mod))%mod)%mod);
                }   
            }
        }
    ...
    inline Matrix recursiveModPow(int p, ll &mod) {
        if (p == 0)
            return Matrix(nrow, ncol, true);
        else if (p == 1)
            return *this;
        else if (p % 2 == 0) {
            *this = this->modMult(*this, mod);
            return recursiveModPow(p/2, mod);
        } else {
            Matrix a = *this;
            Matrix b = recursiveModPow(p-1, mod);

            return a.modMult(b, mod);
        }
    }

    // vector<ll> &operator[](unsigned i) {return m[i];}
