...
ll b(ll x, ll y) {
    if (y > x) return 0;
    ll numerator = fact[x];
    ll denom = mult(inv(fact[x-y]), inv(fact[y]));
    return mult(numerator, denom);
}
...
void solve() {
    ...
    ll x = 0;
    ll z = 0;
    forn(i, n) {cin >> a[i]; x += (a[i] == 1); z += (a[i] == 0);}
    if (k == 1) {
        cout << x << endl;
        return;
    }
    ...
    for(ll ones = need; ones <= k; ones++) {
        ll zeros = k - ones;
        ll oways = b(x, ones);
        ll zways = b(z, zeros);
        dbg(oways);
        dbg(zways);
        ans = sum(ans, mult(oways, zways));
    }
...
void init() {
    fact[0] = fact[1] = 1;
    for1(i, MAXN) fact[i] = mult(fact[i-1], i);
}
