...
void solve() {
    ...
    sort(all(ts), greater<dd>());
    double ratio = d/h;
    dbg(h, d, ratio);
    for1(i, n) {
        ans += area(h, d);
        double b = ts[i].fst, hh = b+h;
        double prevb = ts[i-1].fst, prevhh = prevb+h; 
        if (intersect(b, hh, prevb, prevhh)) {
            double nh = prevhh - hh;
            dbg(h - nh, area(h-nh, h-nh));
            ans -= area(h - nh, d - ratio*nh);
        }
        dbg(ans);
    }
