...
int query(vector<int> loc, int src) {
  string q = "? " + to_string(src) + " " + to_string(sz(loc));
  for (auto x : loc) {
    q += " " + to_string(x);
  }
...
void solve() {
  ...
  vector<int> all_loc;
  for1(i, n + 1) all_loc.pb(i);

  int mxlen = -1;
  int mxi;
  for1(i, n + 1) {
    int len = query(all_loc, i);
    fq[len].pb(i);
    if (len > mxlen) {
      mxi = i;
      mxlen = len;
  ...
  string sans = "! " + to_string(sz(ans));
  for (auto x : ans) {
    sans = sans + " " + to_string(x);
  }
